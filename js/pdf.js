...
async function generateEvaluationPDF(record){
  ...
  const bodyworkDone=countChecklist(record.checklist?.bodywork);
  const bodyworkTotal=Object.values(record.checklist?.bodywork||{}).length||13;

  const interiorDone=countChecklist(record.checklist?.interior);
  const interiorTotal=Object.values(record.checklist?.interior||{}).length||8;

  const mechanicalDone=countChecklist(record.checklist?.mechanical);
  const mechanicalTotal=Object.values(record.checklist?.mechanical||{}).length||14;
  ...
  checklistTwoColumns('3. Lataria e pintura',record.checklist?.bodywork,bodyworkDone,bodyworkTotal);
  checklistTwoColumns('4. Interior e equipamentos',record.checklist?.interior,interiorDone,interiorTotal);
  checklistTwoColumns('5. Mecânica e pneus',record.checklist?.mechanical,mechanicalDone,mechanicalTotal);
